...
class Products
{
	...
	public function addProduct($product_name,
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/OgrenciKafasi/product_images/".$uniqueImageName)) {
					...
					if ($q) {
						return ['status'=> 202, 'message'=> 'Ürün Başarıyla Eklendi..!'];
					}else{
						return ['status'=> 303, 'message'=> 'Sorgu çalıştırılamadı'];
					}

				}else{
					return ['status'=> 303, 'message'=> 'Resim yüklenemedi'];
				}

			}else{
				return ['status'=> 303, 'message'=> 'Büyük resim, izin verilen maksimum boyut 2MB'];
			}

		}else{
			return ['status'=> 303, 'message'=> 'Geçersiz Resim Formatı [Geçerli Formatlar : jpg, jpeg, png]'];
		}
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/OgrenciKafasi/product_images/".$uniqueImageName)) {
					...
					if ($q) {
						return ['status'=> 202, 'message'=> 'Ürün Başarıyla Değiştirildi..!'];
					}else{
						return ['status'=> 303, 'message'=> 'Sorgu çalıştırılamadı'];
					}
	...
	public function editProductWithoutImage($pid,
		...
		if ($pid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Ürün başarıyla güncellendi'];
			}else{
				return ['status'=> 303, 'message'=> 'Sorgu çalıştırılamadı'];
			}

		}else{
			return ['status'=> 303, 'message'=> 'Geçersiz ürün kimliği'];
		}
	...
	public function addCategory($name){
		...
		if ($q->num_rows > 0) {
			return ['status'=> 303, 'message'=> 'Kategori zaten mevcut'];
		}else{
			$q = $this->con->query("INSERT INTO categories (cat_title) VALUES ('$name')");
			if ($q) {
				return ['status'=> 202, 'message'=> 'Yeni kategori başarıyla eklendi'];
			}else{
				return ['status'=> 303, 'message'=> 'Sorgu çalıştırılamadı'];
			}
		}
	...
	public function deleteProduct($pid = null){
		if ($pid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Ürün stoklardan çıkarıldı'];
			}else{
				return ['status'=> 202, 'message'=> 'Sorgu çalıştırılamadı'];
			}
	...
	public function deleteCategory($cid = null){
		if ($cid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Kategori kaldırıldı'];
			}else{
				return ['status'=> 202, 'message'=> 'Sorgu çalıştırılamadı'];
			}

		}else{
			return ['status'=> 303, 'message'=>'Geçersiz kategori kimliği'];
		}
	...
	public function updateCategory($post = null){
		...
		if (!empty($cat_id) && !empty($e_cat_title)) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Kategori güncellendi'];
			}else{
				return ['status'=> 202, 'message'=> 'Sorgu çalıştırılamadı'];
			}
	...
	public function addBrand($name){
		...
		if ($q->num_rows > 0) {
			return ['status'=> 303, 'message'=> 'Sektör zaten mevcut'];
		}else{
			$q = $this->con->query("INSERT INTO brands (brand_title) VALUES ('$name')");
			if ($q) {
				return ['status'=> 202, 'message'=> 'Yeni sektör başarıyla eklendi'];
			}else{
				return ['status'=> 303, 'message'=> 'Sorgu çalıştırılamadı'];
			}
		}
	...
	public function deleteBrand($bid = null){
		if ($bid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Sektör kaldırıldı'];
			}else{
				return ['status'=> 202, 'message'=> 'Sorgu çalıştırılamadı'];
			}

		}else{
			return ['status'=> 303, 'message'=>'Geçersiz sektör kimliği'];
		}
	...
	public function updateBrand($post = null){
		...
		if (!empty($brand_id) && !empty($e_brand_title)) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Sektör güncellendi'];
			}else{
				return ['status'=> 202, 'message'=> 'Sorgu çalıştırılamadı'];
			}
...
if (isset($_POST['add_product'])) {
	...
	&& !empty($_FILES['product_image']['name'])) {
		...
		echo json_encode(['status'=> 303, 'message'=> 'Boş alanlar']);
		exit();
	}
...
if (isset($_POST['add_category'])) {
	if (isset($_SESSION['admin_id'])) {
		...
		echo json_encode(['status'=> 303, 'message'=> 'Oturum Hatası']);
	}
}
...
if (isset($_POST['DELETE_PRODUCT'])) {
	...
	if (isset($_SESSION['admin_id'])) {
		if(!empty($_POST['pid'])){
			...
			echo json_encode(['status'=> 303, 'message'=> 'Geçersiz ürün kimliği']);
			exit();
		}
	}else{
			echo json_encode(['status'=> 303, 'message'=> 'Geçersiz Oturum']);
	}
...
if (isset($_POST['DELETE_CATEGORY'])) {
	if (!empty($_POST['cid'])) {
		...
		echo json_encode(['status'=> 303, 'message'=> 'Geçersiz detaylar']);
		exit();
	}
}
...
if (isset($_POST['DELETE_BRAND'])) {
	if (!empty($_POST['bid'])) {
		...
		echo json_encode(['status'=> 303, 'message'=> 'Gereksiz detaylar']);
		exit();
	}
}
